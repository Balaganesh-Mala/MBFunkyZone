...
const Cart = () => {
  ...
  const handleQty = async (productId, qty, type, size) => {
    if (qty <= 0) return handleRemove(productId, type, size);

    setLoading(true);
    const data = await updateQtyBackend(productId, qty, type, size);
    setLoading(false);
  ...
  const handleRemove = async (productId, type, size) => {
    setLoading(true);
    const data = await removeItemBackend(productId, type, size);
    setLoading(false);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          ...
          {cartItems.map((item) => (
            <div
              ...
              className="flex gap-4 border bg-gray-50 rounded-xl p-4"
            >
              <img
              ...
              <button
                onClick={() =>
                  handleRemove(item.product._id, item.type, item.size)
                }
              >
                <FaTrash />
              </button>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center border-t pt-6 mt-6">
          ...
          <div className="flex gap-3 mt-4 sm:mt-0">
            {cartItems.length > 0 && (
              <button
                onClick={handleClear}
                className="bg-gray-100 border text-xs sm:text-sm px-4 py-2 rounded-full flex items-center gap-2"
              >
                <FaTrash /> Clear Cart
              </button>
            )}
            <button
              ...
              className="bg-black text-white text-xs sm:text-sm px-6 py-2 rounded-full hover:bg-gray-800"
            >
              Checkout
            </button>
          </div>
